...
chmod +x ./jq
sudo cp jq /usr/bin
curl --request POST 'https://api.github.com/repos/acesso-bankly/test-self-hosted/actions/runners/registration-token' --header "Authorization: token ${personal_access_token}" > output.txt
...
~/actions-runner/config.sh --url https://github.com/acesso-bankly/test-self-hosted/ --token \$runner_token --name "Github EC2 Runner" --unattended
